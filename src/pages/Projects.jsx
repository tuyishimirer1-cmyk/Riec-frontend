...
function Projects() {
  ...
  const { isFetching: listFetching, data: projectsData } = useGetProjects(
    { page, limit: pageSize, type, ...(category !== 'all' && { category }), ...(locationFilter && { location: locationFilter }) }
  )
  const { data: searchData, isFetching: searchFetching } = useSearchProjects(
    search, { type, ...(category !== 'all' && { category }), ...(locationFilter && { location: locationFilter }), page, limit: pageSize }
  )
        ...
        <div className="max-w-screen-2xl mx-auto">
          ...
          <div className="mb-10 pb-6 border-b border-gray-700">
            <div className="flex flex-col gap-4 rounded-2xl bg-white/5 p-4 shadow-[0_18px_45px_rgba(15,23,42,0.5)] backdrop-blur">
              ...
              {/* Category Filter and Project Type Toggle Buttons - Same Row */}
              <div className="flex flex-col md:flex-row gap-3 justify-center items-center">
                {/* Category Dropdown */}
                <select value={category} onChange={(e) => { setPage(1); setCategory(e.target.value) }} className="rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-2.5 text-sm text-slate-100 focus:border-riec-orange focus:outline-none">
                ...
                <div className="inline-flex gap-3 bg-slate-900/70 rounded-xl p-2">
                  <button
                    onClick={() => { setPage(1); setType('COMPLETED') }}
                    className={`px-8 py-4 text-sm font-bold rounded-lg transition-all duration-300 transform hover:scale-105 ${
                      type === 'COMPLETED'
                        ? 'bg-gradient-to-r from-riec-orange to-riec-red text-white shadow-xl shadow-riec-orange/50'
                  ...
                  <button
                    onClick={() => { setPage(1); setType('PLAN_TO_BUY') }}
                    className={`px-8 py-4 text-sm font-bold rounded-lg transition-all duration-300 transform hover:scale-105 ${
                      type === 'PLAN_TO_BUY'
                        ? 'bg-gradient-to-r from-riec-orange to-riec-red text-white shadow-xl shadow-riec-orange/50'
                        : 'text-slate-300 hover:text-white hover:bg-slate-800'
          ...
          <div className="flex items-center justify-center gap-4">
            <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1 || isFetching} className="rounded-full border border-slate-600 px-6 py-3 text-sm font-semibold text-slate-100 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-800 transition-colors">
              Previous
            </button>
            <button type="button" onClick={() => setPage((p) => (p < totalPages ? p + 1 : p))} disabled={page >= totalPages || isFetching} className="rounded-full border border-slate-600 px-6 py-3 text-sm font-semibold text-slate-100 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-800 transition-colors">
              Next
            </button>
          </div>
        </div>
